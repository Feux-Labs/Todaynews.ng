    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">Live articles visible on Todaynews.ng (Sorted by Date Newest First)</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30 cursor-pointer"
            >
              <option value="newest" className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">📅 Newest First (Date)</option>
              <option value="oldest" className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">⏳ Oldest First (Date)</option>
              <option value="views" className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">🔥 Most Views</option>
              <option value="title" className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">🔤 Headline (A-Z)</option>
            </select>
          </div>

          {/* Page Size Selector */}
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">Per Page:</span>
            <select
              value={pageSize}
              onChange={(e: any) => setPageSize(e.target.value === "all" ? "all" : Number(e.target.value))}
              className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30 cursor-pointer"
            >
              <option value={10} className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">10 per page</option>
              <option value={25} className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">25 per page</option>
              <option value={50} className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">50 per page</option>
              <option value={100} className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">100 per page</option>
              <option value="all" className="bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white">See All ({articles.length})</option>
            </select>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white placeholder:text-slate-500 dark:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30 w-52 sm:w-64"
            />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <Globe className="w-12 h-12 text-slate-600 dark:text-slate-400 mx-auto mb-3" />
          <p className="text-slate-400 dark:text-slate-500">No published articles found</p>
        </div>
      ) : (
        <div className="space-y-4">
            ...
            >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 px-3 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white text-xs focus:outline-none focus:ring-2 focus:ring-[#2563eb]/30 font-mono"
                        />
                        <button
                          type="button"
                          onClick={() => openImageModal(article)}
                          className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs rounded-lg flex items-center gap-1 font-medium transition"
                        >
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        <button onClick={() => setEditingId(null)} className="px-3 py-1.5 bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 text-xs rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                          Cancel
                        </button>
                      </div>
                    </div>
                    ...
                    <>
                      <h3 className="text-base font-semibold text-slate-900 dark:text-white leading-tight flex items-center gap-2">
                        ...
                        <Link
                          ...
                          target="_blank"
                          className="text-slate-500 dark:text-slate-400 hover:text-[#2563eb] transition"
              ...
              {editingId !== article.id && (
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 items-center">
                  <button onClick={() => startEdit(article)} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-medium rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition">
          ...
          {pageSize !== "all" && totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-slate-200 dark:border-slate-700 text-xs text-slate-400 dark:text-slate-500">
              <div>
                Showing <span className="text-slate-900 dark:text-white font-bold">{startIndex + 1}</span> to{" "}
                <span className="text-slate-900 dark:text-white font-bold">{endIndex}</span> of{" "}
                <span className="text-slate-900 dark:text-white font-bold">{filtered.length}</span> published articles
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg font-medium hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-1 text-slate-900 dark:text-white transition"
                >
          ...
          <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white text-xs placeholder-slate-500 focus:outline-none focus:border-[#2563eb]/50 font-mono"
              />
            </div>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 p-1.5 rounded-lg border text-left text-[11px] transition ${
                      newImageUrl === preset.url
                        ? "bg-[#2563eb]/15 border-[#2563eb] text-slate-900 dark:text-white font-semibold"
